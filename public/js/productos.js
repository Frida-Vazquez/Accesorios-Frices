...
function editProducto(id, productos) {
  ...
  // Mostrar la imagen actual en la vista previa, si existe
  const previewContainer = document.getElementById("prodImagenPreview");
  const previewImg = document.getElementById("prodImagenPreviewImg");
  const fileText = document.getElementById("prodImagenTexto");

  if (form.dataset.currentImage && previewImg && previewContainer) {
    previewImg.src = form.dataset.currentImage;
    previewContainer.classList.remove("hidden");
    if (fileText) fileText.textContent = "Imagen actual seleccionada";
  } else {
    if (previewContainer) previewContainer.classList.add("hidden");
    if (fileText) fileText.textContent = "Sin archivos seleccionados";
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Cambiar texto y vista previa cuando se selecciona archivo
  if (fileInput) {
  ...
  if (btnNuevoProducto) {
    btnNuevoProducto.addEventListener("click", () => {
      ...
      // limpiar texto y preview de imagen
      if (fileText) fileText.textContent = "Sin archivos seleccionados";
      if (previewContainer) previewContainer.classList.add("hidden");
