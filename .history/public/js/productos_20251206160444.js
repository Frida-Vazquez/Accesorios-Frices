  ...
  try {
    ...
    lista.forEach((p) => {
      ...
      tr.innerHTML = `
        ...
        <td class="py-2 pr-2">${p.descripcion || ""}</td>
...
function editProducto(id, productos) {
  ...
  document.getElementById("prodColor").value = p.color_principal || "";
  document.getElementById("prodLargo").value = p.largo_cm ?? "";

  document.getElementById("prodDestacado").checked = !!p.destacado;
  document.getElementById("prodHipo").checked =
    p.es_hipoalergenico === 1 || p.es_hipoalergenico === true;
  document.getElementById("prodAjustable").checked =
    p.es_ajustable === 1 || p.es_ajustable === true;
  document.getElementById("prodActivo").checked =
    p.activo === 1 || p.activo === true;

  // guardar imagen actual si ya existe
...
document.addEventListener("DOMContentLoaded", () => {
  if (!getToken()) return handleAuthError();
  ...
  form.addEventListener("submit", async (e) => {
    ...
    formData.append("descripcion", document.getElementById("prodDescripcion").value.trim());
    formData.append("precio", document.getElementById("prodPrecio").value || 0);
    ...
    formData.append("tipo_pieza", document.getElementById("prodTipoPieza").value.trim());
    formData.append("material", document.getElementById("prodMaterial").value.trim());
    formData.append("color_principal", document.getElementById("prodColor").value.trim());
    formData.append("largo_cm", document.getElementById("prodLargo").value || "");
    ...
    formData.append(
      "es_hipoalergenico",
      document.getElementById("prodHipo").checked ? "1" : "0"
    );
    formData.append(
      "es_ajustable",
      document.getElementById("prodAjustable").checked ? "1" : "0"
    );
    formData.append(
      "activo",
      document.getElementById("prodActivo").checked ? "1" : "0"
    );

    // imagen actual (si edito y no cambio archivo)
