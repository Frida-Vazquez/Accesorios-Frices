...
function clearClienteSession() {
  localStorage.removeItem(CLIENTE_TOKEN_KEY);   // frices_token
  localStorage.removeItem(CLIENTE_NOMBRE_KEY);  // cliente_nombre
...
function initUserMenu() {
  ...
  if (!token) {
    ...
    // Clic en el icono → manda al login
    userMenuBtn.addEventListener("click", () => {
      window.location.href = "/static/auth/login.html";
    });

    // Menú se queda oculto
    return;
  }
  ...
  logoutBtn.addEventListener("click", () => {
    clearClienteSession();
    // Después de cerrar sesión, vuelve a la página del cliente (sin sesión)
