...
function initUserMenu() {
  ...
  // Si hay sesión → mostramos "Hola, Frida" y el menú
  const primerNombre = (nombre || "").split(" ")[0] || "Cliente";

  if (greetingEl) {
    greetingEl.textContent = `Hola, ${primerNombre}`;
  }
  ...
  logoutBtn.addEventListener("click", () => {
    clearClienteSession();
    window.location.href = "/static/auth/login.html"; // o a inicio si quieres
  });
}
// ========== MENÚ DEL USUARIO (CLIENTE) ==========
function initUserMenu() {
  ...
  if (!token) {
    ...
    // Al hacer clic en el icono → ir al login
    userMenuBtn.addEventListener("click", () => {
      window.location.href = "/static/auth/login.html";
    });

    // Nos salimos, no activamos menú desplegable
    return;
  }
  ...
  // Cerrar sesión
  logoutBtn.addEventListener("click", () => {
    clearClienteSession(); // borra token y nombre
    window.location.href = "/static/auth/login.html"; // regresa a login
  });
}
