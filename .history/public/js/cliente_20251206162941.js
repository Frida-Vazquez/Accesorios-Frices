...
async function loadCategorias() {
  try {
    ...
    if (categorias.length === 0) {
      grid.innerHTML = `
        <p class="col-span-4 text-center text-sm text-gray-500">
    ...
    categorias.forEach((cat) => {
      ...
      const imagen =
        cat.imagen_url || cat.imagen || cat.foto || "https://via.placeholder.com/400x500?text=Categoria";

      card.innerHTML = `
        <div class="w-full aspect-[4/5] overflow-hidden">
          <img src="${imagen}" alt="${cat.nombre || "Categoría"}"
