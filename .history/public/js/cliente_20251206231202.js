...
async function loadCategorias() {
  ...
  try {
    ...
    // data debe ser un array de categorías: { id, nombre, slug, url_imagen, ... }
    data.forEach((cat) => {
      // si no tienes slug en la BD, lo generamos a partir del nombre
      const slug =
        cat.slug ||
        cat.nombre
          .toLowerCase()
      ...
      // 🔥 AQUÍ ESTÁ LA MAGIA: al hacer clic, vamos a /collections/slug
      ...
      article.innerHTML = `
        <div class="w-full aspect-[4/5] overflow-hidden">
          <img
            src="${cat.url_imagen || "https://via.placeholder.com/600x800?text=Categoria"}"
            alt="${cat.nombre}"
