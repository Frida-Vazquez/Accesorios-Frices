...
const CLIENTE_TOKEN_KEY = "frices_carrito"; // 👈 NUEVA CONSTANTE
...
async function agregarAlCarrito(productoId) {
  if (!getClienteToken()) {
    ...
    window.location.href = "/static/auth/login.html"; // ajusta ruta si tu login es otra
    return;
  }
  ...
  try {
    ...
    if (!resp.ok) throw new Error("Error al agregar producto al carrito");
...
function toggleFavorito(id) {
    ...
    setFavoritos(favs);
    return favs; // devolvemos la lista actualizada
}
    ...
    lista.forEach((p) => {
      ...
      <div class="p-4 flex-1 flex flex-col">
        ...
        <div class="mt-auto flex items-center justify-between">
          ...
          <button
  class="btn-agregar text-xs px-3 py-1 rounded-full border border-[#8a555b] text-[#8a555b] hover:bg-[#8a555b] hover:text-white transition"
>
  Agregar
</button>
    ...
    `;
        ...
        if (btnAgregar) {
            btnAgregar.addEventListener("click", (e) => {
                e.stopPropagation(); // evita que abra la vista detalle

                agregarAlCarrito(p.id); // 👈 AGREGA EL PRODUCTO AL CARRITO REAL
            });
        }
...
function aplicarFiltros() {
    ...
    switch (sort) {
        ...
        case "fecha-desc":
            ...
            break;
        case "mas-vendidos":
            // sin campo de ventas, usamos stock como aproximación
            lista.sort(
