...
function setMessage(msg, type = "success") {
  ...
  // Resetear estilos base (Tailwind)
  box.className = "text-sm rounded-2xl px-4 py-3 mt-1";
...
function initTabs() {
  ...
  buttons.forEach((btn) => {
    btn.addEventListener("click", () => {
      // estilo del botón activo (si quieres, puedes añadir/quitar clases Tailwind aquí)
  ...
  // Asegurar que al inicio solo se vea clientes
  sections.forEach((sec) => {
    if (sec.id === "clientesSection") {
      sec.classList.remove("hidden");
    } else {
      sec.classList.add("hidden");
    }
