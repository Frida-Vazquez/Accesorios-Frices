...
async function loadProductos() {
    try {
        ...
        <td class="py-2 pr-2">${p.tipo_pieza || ""}</td>
        <td class="py-2 pr-2">${p.material || ""}</td>
        <td class="py-2 pr-2">${p.color_principal || ""}</td>
        <td class="py-2 pr-2">${p.es_hipoalergenico ? "Sí" : "No"}</td>
        <td class="py-2 pr-2">${p.es_ajustable ? "Sí" : "No"}</td>
        <td class="py-2 pr-2">${p.largo_cm ?? ""}</td>
        <td class="py-2 pr-2">${p.activo ? "Sí" : "No"}</td>
        <!-- FIN NUEVOS CAMPOS EN TABLA -->
...
function editProducto(id, productos) {
    ...
    document.getElementById("prodColorPrincipal").value = p.color_principal || "";
    document.getElementById("prodHipoalergenico").checked = !!p.es_hipoalergenico;
    document.getElementById("prodAjustable").checked = !!p.es_ajustable;
    document.getElementById("prodLargoCm").value = p.largo_cm ?? "";
    document.getElementById("prodActivo").checked = p.activo === 0 ? false : true;
    // FIN NUEVOS CAMPOS
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Botón logout (si existiera en el header)
    ...
    form.addEventListener("submit", async (e) => {
        ...
        try {
            ...
            document.getElementById("prodId").value = "";
            delete form.dataset.currentImage;
            closeModalProducto(); // cerrar modal al guardar
            loadProductos();
        } catch (err) {
    ...
    // Limpiar formulario (pero sin cerrar el modal)
