...
formData.append("precio", Number(document.getElementById("prodPrecio").value || 0));
formData.append("categoria_id", document.getElementById("prodCategoria").value);
formData.append("stock", Number(document.getElementById("prodStock").value || 0));
formData.append("destacado", document.getElementById("prodDestacado").checked);

// agregar la imagen
const fileInput = document.getElementById("prodImagen");
if (fileInput.files.length > 0) {
  formData.append("imagen", fileInput.files[0]); // <-- archivo local
}

const res = await fetch(url, {
  method,
  headers: {
    ...authHeader(), // ❗ NO agregues Content-Type aquí
  },
  body: formData,
});
